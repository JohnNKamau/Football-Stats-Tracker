...
export async function fetchTeams() {
  const res = await fetch(`${BASE_URL}/teams?league=39&season=2024`, {
...
export async function fetchFixtures() {
  const res = await fetch(`${BASE_URL}/fixtures?league=39&season=2024`, {
    headers:  { "x-apisports-key": API_KEY }
  });
  const data = await res.json();
  return data.response;
...
export async function fetchPlayer(name) {
  const res = await fetch(`${BASE_URL}/players?search=${name}&season=2024&league=39`, {
    headers: { "x-apisports-key": API_KEY }
  });
  const data = await res.json();
  return data.response || [];
